...
if pgrep -x "rofi" > /dev/null; then
    ...
    # Listar dispositivos de áudio com a saída simplificada (apenas nomes)
    devices=$(pactl list sinks short | awk '{print $2}')

    # Obter o dispositivo de saída padrão
    default_device=$(pactl info | grep "Default Sink" | awk -F': ' '{print $2}')
    ...
    selected_device=$(echo -e "$devices_with_marker" | rofi -dmenu -p "Selecione o dispositivo de áudio" -theme Arc-Dark)
